      ...
      <main className="flex-1 container max-w-7xl mx-auto px-6 py-12">
        ...
        {loading && assets.length === 0 ? (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {[...Array(8)].map((_, i) => (
              <Skeleton key={i} className="aspect-square rounded-md bg-surface-soft" />
            ))}
          </div>
          ...
          <div className="space-y-12">
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {filteredAssets.map((asset) => (
                <div
                  key={asset.public_id}
                  className="group relative bg-canvas rounded-md border border-hairline overflow-hidden hover:border-primary/50 transition-all duration-300"
                >
